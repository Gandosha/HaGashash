...
func WebScan(protocol string, targetIP string, outputPath string, port2scan string) {
	...
	switch {
		case fileInfo.Size() == 0:
			...
			niktoCmd := exec.Command("bash", "-c", "nikto -h " + protocol + "://" + targetIP + ":" + port2scan + " -ask no >> " + outputPath + "/nikto_scan_out_" + port2scan + "_" + protocol)
			err = niktoCmd.Run()
		    	if err != nil {
				panic(err)
		    	}
		case fileInfo.Size() > 0:
			...
			for scanner.Scan() {
				...
				niktoCmd := exec.Command("bash", "-c", "echo -e '\n\n" + protocol + "://" + targetIP + ":" + port2scan + scanner.Text() + "\n\n' >> " + outputPath + "/nikto_scan_out_" + port2scan + "_" + protocol + " && nikto -h " + protocol + "://" + targetIP + ":" + port2scan + scanner.Text() + " -mutate 6 -mutate-options " + outputPath + "/gobuster_wordlist_" + port2scan + "_" + protocol + " -ask no >> " + outputPath + "/nikto_scan_out_" + port2scan + "_" + protocol)
			    	err = niktoCmd.Run()
			    	if err != nil {
					panic(err)
			    	}
