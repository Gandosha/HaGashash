...
func main() {	
	...
	hostsPtr := flag.String("hosts", "nil", "Scan only ip addresses that are mentioned in the list (Specify path for the file. Ex. /root/temp/targets).")
	...
	switch {
		...
		case *projectNamePtr == "nil":
			...
			fmt.Println("\n")
			os.Exit(1)
		case *hostPtr == "nil" && *subnetPtr == false && *subnetsPtr == false && *targetListPtr == "nil":
