...
func main() {	
	...
	subnetPtr := flag.Bool("subnet", "nil", "Discover alive hosts in subnet and scan them.")
	...
	case *projectNamePtr == "nil":
		...
		fmt.Println("\n")
		os.Exit(1)
	case *hostPtr == "nil" && *subnetPtr == "nil":
	...
	case *interfacePtr != "nil" && *hostPtr != "nil" && *projectNamePtr != "nil" && *subnetPtr == "nil":
		fmt.Println("\n[!] Starting to scan " + *hostPtr)	
		var tarsTCPorts, tarsUDPorts []string
